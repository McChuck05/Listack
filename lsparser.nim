...
import std/strutils, std/deques, std/terminal

proc parseCommands*(originalText: string, debug: bool = false, verbose: bool = false): LsSeq = 
  var 
    parsed: LsSeq
  ...
  proc doFail(message, detail: string) =
    ...
    quit("Parsing halted due to error\n")
  ...
  proc doEscapeChar(): char =
    ...
    of 'b': result =  '\b'
    of 'e': result =  '\e'
    else: doFail("Unrecognized escape character", ("\\" & $thisChar))
    return result
  ...
  proc initSeq(kind: LsType, deferred: bool) = 
  ...
  proc handlePrefixedWord() =
    ...
    if thisChar == '@':   # interact with name
      ...
      fixMarker = Postfix
      thisChar = getNext()
    if thisChar in digits and functionChar in {'<', '>', '!', '*', '?', '/'}:  # need to handle '@>1'
      node = newWordNode($thischar, Immediate, functionChar, line, column)
      done = true
      thisChar = getNext()
      skipWhite()
    elif thisChar notin wordstart:
      if functionChar != '\0':
        doFail("Syntax error, @" & $functionChar & " only applies to names", $thisChar)
      ...
      if node.nodeType != Null: # account for sugar words being ignored
        ...
        if functionChar != '\0':
          if node.wordFix != Infix or fixMarker == Postfix:
            doFail("Syntax error, @" & $node.wordFunc & " only applies to names", $node)
  ...
  try:
    ...
    doFail("Type error", getCurrentExceptionMsg())

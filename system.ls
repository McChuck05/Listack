...
def: "each" [Coll, Word] {>block .each}  # [1 2 3] \println .each
...
def: "map" [Coll, Blocky]
  ...
  .each
  depth @<_map_collect .- dup 0 .>= {.enlist} {drop [] >bad} .if}

def: "map" [Coll, Word] {>block .map}
def: "map" [Item, Blocky] {swap >list swap .map}
def: "map" [Item, Word] {>block swap >list swap .map}
...
create_global: "_apply_n"
def_immediate: "a_b_apply_n" [Any, Any, Int, Int, Executable]
  {@>_apply_n @>_apply_b @>_apply_a 
  _apply_a + _apply_b <= depth if: { 
    @<_apply_b {> 1} {.enlist} .iff* @>_apply_b 
    @<_apply_a {> 1} {.enlist} .iff*
    @<_apply_b @<_apply_n eval}
  else {set_err: "a_b_apply: insufficient items on stack"} }

def_immediate: "bi_each" [Any, Executable, Executable] {1 2 \.apply_each a_b_apply_n}
def_immediate: "bi_map" [Any, Executable, Executable] {1 2 \.apply_map a_b_apply_n}


# times
create_global: "_times_counter"
def: "times" [Blocky, Int]
  {dup 0 .>   # if
    {@>_times_counter {@<_times_counter dec dup @>_times_counter 0 .>=} swap .while 
      @<_times_counter drop}
    {drop2} .if}

def: "times" [Word, Int] {swap >block swap .times}
def: "times" [Int, Blocky] {swap .times}
def: "times" [Int, Word] {>block swap .times}
...
def: "filter" [Coll, Word] {>block .filter}
...
def_immediate: "dip" [Any, Blocky] {swap @>_dipped eval @<_dipped}
def_immediate: "dip" [Any, Word] {dip}

# a b {commands} keep --> a b commands b
def_immediate: "keep" [Any, Blocky] {over @>_dipped eval @<_dipped}
def_immediate: "keep" [Any, Word] {keep}

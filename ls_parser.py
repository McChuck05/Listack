...
def parse_commands(line, is_meta = False, debug = False):
    original = line
    line = line.strip()
    error = False
    parsed = deque([])
    ...
    if debug:
        print(f"\n\nParsing: {Fore.BLUE}{line}{Style.RESET_ALL}\n")

    try:
        while line:
            if debug:
                print(f"Word: {Fore.YELLOW}{word}{Fore.WHITE} Current: {Fore.RED}{current}{Style.RESET_ALL}", flush=True)
            previous = current
            ...
            if b_solo:      # is this a lone backquote?
                ...
                if current in delim:
                    ...
                    err = f"{Fore.RED}{Style.BRIGHT}Error:{Style.NORMAL} ` only applies to words, not: {Fore.YELLOW}{word}{current}{Style.RESET_ALL}"
                    print(err)
                    raise ValueError
                else:
                    in_word = True
            ...
            if in_block:        # {}
                if current == '"' and previous != "\\":
                    ...
                    if block_depth > 10000:
                        ...
                        err = f"{Fore.RED}{Style.BRIGHT}Error:{Style.NORMAL} block depth exceeded 10,000: {Fore.YELLOW}{word}{current}{Style.RESET_ALL}"
                        print(err)
                        raise ValueError
                elif current == "[":
                    list_depth += 1
                    if list_depth > 10000:
                        ...
                        err = f"{Fore.RED}{Style.BRIGHT}Error:{Style.NORMAL} list depth exceeded 10,000: {Fore.YELLOW}{word}{current}{Style.RESET_ALL}"
                        print(err)
                        raise ValueError
                elif current == "]":
                    list_depth -= 1
                    if list_depth < 0:
                        ...
                        err = f"{Fore.RED}{Style.BRIGHT}Error:{Style.NORMAL} attempt to close unopened list: {Fore.YELLOW}{word}{current}{Style.RESET_ALL}"
                        print(err)
                        raise ValueError
                elif current == "}":
                    block_depth -= 1
                    if block_depth < 0:      # not sure how you got here
                        ...
                        err = f"{Fore.RED}{Style.BRIGHT}Error:{Style.NORMAL} attempt to close unopened block: {Fore.YELLOW}{word}{current}{Style.RESET_ALL}"
                        print(err)
                        raise ValueError
                    elif block_depth == 0:
                        in_block = False
                        returned = parse_commands(word, is_meta)
            ...
            if in_var:
                if len(word) == 1:
                    if current.isalpha():
                        ...
                        err = f"{Fore.RED}{Style.BRIGHT}Meta error:{Style.NORMAL} variables must be a letter: {Fore.YELLOW}{word}{current}{Style.RESET_ALL}"
                        print(err)
                        raise ValueError
                    continue
                elif len(word) == 2:
                ...
                if current in delim or current.isspace() or current in [".", ":"]:  # '.' only at beginning, ":" only at end of varnames
                    ...
                    if word in sugar:
                        ...
                        if is_valid_word(word):
                            ...
                            # print(f"\n{Fore.RED}Parsing error: {Fore.MAGENTA}{word}{Fore.YELLOW} is not a valid word{Style.RESET_ALL}", flush = True)
                            err = f"\n{Fore.RED}Parsing error: {Fore.MAGENTA}{word}{Fore.YELLOW} is not a valid word{Style.RESET_ALL}"
        ...
        if in_d_string or in_s_string:
            ...
            err = f"{Fore.RED}{Style.BRIGHT}Error:{Style.NORMAL} unclosed string: {Fore.YELLOW}{word}{Style.RESET_ALL}"
            error = True
        if in_block:
           #  print(f"{Fore.RED}{Style.BRIGHT}Error:{Style.NORMAL} unclosed block: {Fore.YELLOW}{word}{Style.RESET_ALL}")
            err = f"{Fore.RED}{Style.BRIGHT}Error:{Style.NORMAL} unclosed block: {Fore.YELLOW}{word}{Style.RESET_ALL}"
            error = True
        if in_list:
            # print(f"{Fore.RED}{Style.BRIGHT}Error:{Style.NORMAL} unclosed list: {Fore.YELLOW}{word}{Style.RESET_ALL}")
            err = f"{Fore.RED}{Style.BRIGHT}Error:{Style.NORMAL} unclosed list: {Fore.YELLOW}{word}{Style.RESET_ALL}"
            error = True
        if in_num:
            parsed.append(int(word))
        if in_float:
        ...
        if current.isspace():
            current = ""
        print(f"{Fore.GREEN}{consumed}{Fore.YELLOW}\n\n>>> {Fore.MAGENTA}{word}{Fore.RED}{current}{Fore.YELLOW} <<<\n{err}\n\n{Fore.CYAN}{line}{Style.RESET_ALL}\n")

...
def main_loop(commands_in, debug):
    ...
    try:
        while ls.future:
            ...
            if is_seq(current):
            ...
            elif current[0] == "$":     # quick alias for $varname.set
                push_right(ls.past, current[1:])
                ls.do_set()

            elif current[0] == "@":     # quick alias for @varname.get
                push_right(ls.past, current[1:])
                ls.do_get()
            ...
            elif current == "then": # syntactic sugar
                continue

            elif current == "else": # syntactic sugar
                continue
            ...
            elif current == "do":   # syntactic sugar
                continue
            ...
            elif current == ".map":      # postfix   list block .map    like EACH, but collects values into a list    uses side stack e
                ls.do_map()
            ...
            elif current == ".times":   # n {block} .times --> execute {block} n times, n stored in side_n      uses side stack t
                ls.do_times()
                ...
                ls.do_for()     # counter is TOS for body to use
...
# Math
            ...
            elif current in [".^", ".pow"]:    # a b .^ --> a**b exponent or power
                ls.do_power()

            elif current in ["^:", "pow:"]:   # ^: a b --> a**b
                ls.do_power_pre()

            elif current in ["^", "pow"]:    # a ^ b --> a**b
                ls.do_power_in()

            elif current in [".^/", ".root"]:   # a b .root --> b'th root of a --> a**(1/b)
                ls.do_root()

            elif current in ["^/:", "root:"]:   # root: a b
                ls.do_root()

            elif current in ["^/", "root"]:   # a root b
                ls.do_root()
...
# Boolean logic
            ...
            elif current == "_swap ff": # swap top two items on the future queue
                ls.do_swap_ff()

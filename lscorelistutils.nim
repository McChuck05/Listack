...
proc doLenKeep*(env: var Environment) =
  ...
  if env.past[^1][^1].nodeType in collTypes:
...
proc doNthS*(env: var Environment) =
  var invalid = checkInvalid(env, 2)
  var where = env.past[^1].popLast().intVal
  var target = env.past[^1][^1].stringVal  # this is a copy, not a ref
  if len(target) == 0:
...
proc doToCharI*(env: var Environment) =
  ...
  var what: char
  if this < 0 or this > 255:
    handleError(env, ">char only woks on integers between 0 and 255, not: " & $this, env.current)
    invalid = true
    what = '\0'
  else:
    what = char(this)
...
proc doToNumChar*(env: var Environment) = # "42" >num --> 42
  var invalid = checkInvalid(env, 1)
...
proc doUnZip*(env: var Environment) =
  ...
  if len(zipped)  > 0:
    for pairing in zipped:
      if pairing.nodeType in collTypes and len(pairing) == 2:
...
core["<in"] = CoreVars(count: 2)      # [1 2 3] <in 2 --> true
proc doInRev*(env: var Environment) =   # <in: [1 2 3] 1 --> true
  var invalid = checkInvalid(env, 2)
  var what = env.past[^1].popLast()
  var target = env.past[^1].popLast()
  var found = false
  ...
  env.past[^1].addLast(newBoolNode(found))
  setInvalid(env, invalid)
core["<in"].variant.add(CoreObj(args: @["Coll", "Item"], cmd: doInRev))
core["<in"].variant.add(CoreObj(args: @["Coll", "Coll"], cmd: doInRev))

proc doInRevSC*(env: var Environment) =   # "string" <in `i --> true
  var invalid = checkInvalid(env, 2)
  var what = env.past[^1].popLast()
  var target = env.past[^1].popLast()
  var found = false
  ...
  env.past[^1].addLast(newBoolNode(found))
  setInvalid(env, invalid)
core["<in"].variant.add(CoreObj(args: @["String", "Char"], cmd: doInRevSC))

proc doInRevSS*(env: var Environment) =   # "string" <in `i --> true
  var invalid = checkInvalid(env, 2)
  var what = env.past[^1].popLast()
  var target = env.past[^1].popLast()
  var found = false
  ...
  env.past[^1].addLast(newBoolNode(found))
  setInvalid(env, invalid)
core["<in"].variant.add(CoreObj(args: @["String", "String"], cmd: doInRevSS))
...
core["<where"] = CoreVars(count: 2)      # [1 2 3] <where 2 --> 1
proc doWhereRev*(env: var Environment) =
  var invalid = checkInvalid(env, 2)
  var what = env.past[^1].popLast()
  var target = env.past[^1].popLast()
  var where = -1
  ...
  env.past[^1].addLast(newIntNode(where))
  setInvalid(env, invalid)
core["<where"].variant.add(CoreObj(args: @["Item", "Coll"], cmd: doWhereRev))
core["<where"].variant.add(CoreObj(args: @["Coll", "Coll"], cmd: doWhereRev))

proc doWhereRevSC*(env: var Environment) =   # "string" <where `r --> 3
  var invalid = checkInvalid(env, 2)
  var what = env.past[^1].popLast().charVal
  var target = env.past[^1].popLast().stringVal
  var found = -1
  ...
  env.past[^1].addLast(newIntNode(found))
  setInvalid(env, invalid)
core["<where"].variant.add(CoreObj(args: @["String", "Char"], cmd: doWhereRevSC))

proc doWhereRevSS*(env: var Environment) =   # "string" <where "r" --> 2
  var invalid = checkInvalid(env, 2)
  var what = env.past[^1].popLast().stringVal
  var target = env.past[^1].popLast().stringVal
  var found = -1
  ...
  env.past[^1].addLast(newIntNode(found))
  setInvalid(env, invalid)
core["<where"].variant.add(CoreObj(args: @["String", "String"], cmd: doWhereRevSS))

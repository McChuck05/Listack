...
class ls_stack(deque):
    pass

def make_bool(item):
    return item not in [None, False, 0.0, 0, "", (), {}, [], ls_word(""), ls_block([]), ls_list([]), ls_word("False"), ls_word("false"), ls_word("FALSE")]

def is_deq(seq):
    return type(seq) is deque

def is_list(seq):
    return type(seq) is ls_list

def is_block(seq):      # {}
    return type(seq) is ls_block

def is_seq(seq):
    return type(seq) in [ls_block, ls_list, deque, list]

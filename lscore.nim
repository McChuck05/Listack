...
import std/deques, std/tables, std/strutils, std/terminal, std/algorithm
...
core["word_def"] = CoreVars(count:1)  
proc doFuncBody*(env: var Environment) =
  let funcNameNode = env.past[^1].popLast()
  var funcName: string
  if funcNameNode.nodeType == String:
    funcName = funcNameNode.stringVal
  else:
    funcName = funcNameNode.wordVal
  var totalFuncSeq = newLsSeq()
  var thisFuncSeq = newLsSeq()
  if core.hasKey(funcName):
    for variant in core[funcName].variant:
      thisFuncSeq = newLsSeq()
      thisFuncSeq.addFirst(newStringNode(funcName & ";core"))
      thisFuncSeq.addLast(newListNode())
      for thisType in variant.args:
        thisFuncSeq[^1].seqVal.addLast(newStringNode(deepCopy(thisType)))
      thisFuncSeq.addLast(newWordNode(funcName))
      if funcName in immediates or core[funcName].count < 0:
        thisFuncSeq[^1].wordFix = Immediate
      elif core[funcName].count == 0:
        thisFuncSeq[^1].wordFix = Infix
      totalFuncSeq.addLast(newBlockNode(thisFuncSeq))
  for namespc, val in env.nameSpace:
    if val.hasKey(funcName):
      for variant in val[funcName].variant:
        thisFuncSeq = newLsSeq()
        thisFuncSeq.addFirst(newStringNode(funcName & ";" & namespc))
        thisFuncSeq.addLast(newListNode())
        for thisType in variant.args:
          thisFuncSeq[^1].seqVal.addLast(newStringNode(deepCopy(thisType)))
        thisFuncSeq.addLast(deepCopy(variant.cmd))
        totalFuncSeq.addLast(newBlockNode(thisFuncSeq))
  env.past[^1].addLast(newListNode(totalFuncSeq))
core["word_def"].variant.add(CoreObj(args: @["Wordy"], cmd: doFuncBody))


core["words"] = CoreVars(count:0)  
proc doWords*(env: var Environment) =
  stdout.styledWriteLine(bgRed, "core")
  var funcList: seq[string]
  for funcName, funcVars in core:
    funcList.add(funcName)
  sort(funcList)
  for count, this in funcList:
    stdout.write(this, "  ")
    if (count + 1) mod 10 == 0:
      echo ""
  echo "\n"
  for namespc, cmdList in env.nameSpace:
    stdout.styledWriteLine(bgRed, nameSpc)
    funcList = @[]
    for funcName, variants in cmdList:
      funcList.add(funcName)
    sort(funcList)
    for count, this in funcList:
      stdout.write(this, "  ")
      if (count + 1) mod 10 == 0:
        echo ""
    echo "\n"
core["words"].variant.add(CoreObj(args: @[], cmd: doWords))

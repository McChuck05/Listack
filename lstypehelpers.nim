import std/deques, std/math, std/algorithm
...
proc newWordNode*(val: string = "", fix: LsFix = PostFix, wFunc: char = '\0', line: Natural = 0, column: Natural = 0): LsNode =
  return LsNode(nodeType: Word, wordVal: val, wordFix: fix, wordFunc: wFunc, wordLine: line, wordColumn: column, deferred: false, invalid: false)
...
proc newObjectNode*(kind: LsSeq = newLsSeq(), args: LsSeq = newLsSeq(), val: LsNode = newNullNode()): LsNode =
...
proc getSeq*(target: LsNode): LsSeq = 
  if target.nodeType in collTypes:
    return target.seqVal
  elif target.nodeType == Object and target.objectVal.nodeType in collTypes:

    ...
    try:
        while ls.future:
            ...
            if debug:
                ...
                my_println(immediate_future, True)
                print("\n", flush=True)
                try:
                    getch()
...
# combinators
                ...
                ls.do_reduce()           # uses side stacks t, u, e (each)
...
# list functions
            ...
            elif current == ".range":   # start stop .range --> [start, next, next ... stop]     works for numbers and characters
                ls.do_range()

            elif current == "range:":   # range: start stop
                ls.do_range_pre()

            elif current == "range":    # start range stop
                ls.do_range_in()
...
# anything else is a fatal error
        ...
        print(f"\n{Fore.MAGENTA}Command Queue: {Style.RESET_ALL}", flush=True, end="")
        partial = copy_of(ls.future)
        partial = ls_slice(partial, 12)
        my_println(partial, True)
        print("\nMore detail? [y/N] ", flush=True, end = "")
        response = getche()
        if response in ["y", "Y"]:
            ...
            for c in ls.local_vars[-1]:
                if c not in uppers:
                    print()
                print(f"{Fore.CYAN}{c}{Style.RESET_ALL} = ", end="")
                my_print(ls.local_vars[-1][c], True)
                print("  ", sep="", end="")
            print(f"\n\n{Fore.YELLOW}Side stacks:{Style.RESET_ALL}")
            for c in lowers:
                print(f"{Fore.CYAN}{c}{Style.RESET_ALL} = ", end="")
                my_print(ls.side[c], True)
                print("  ", end="")
            print(f"\n\n{Fore.YELLOW}Global functions{Style.RESET_ALL}")
            for c in ls.global_funcs:
                print(f"{Fore.CYAN}{c}{Style.RESET_ALL} = ", end="")
                my_print(ls.global_funcs[c], True)
                print("\n")
            print()
        raise
...
def main(args):
    ...
    try:
        with open(file_arg, "r") as in_file:
            raw = in_file.read()
            in_file.close()
        parsed = parse_commands(raw, False, debug)

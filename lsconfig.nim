...
var
  immediates* = @["pick", "roll",
    ...
    "depth", "dump", "save_stack", "restore_stack", "copy_stack",
...
proc lsTypeCheck*(passedFormal: string, passedActual: LsType): bool =
  let formal = passedFormal
  let actual = $passedActual
  if formal notin PseudoTypes:
    echo "Type mismatch error: ", formal, " not in list of pesudotypes"
    quit("Type not found ", 65)
  if formal == "Any":
    return true
  if formal == "Item":
    return actual in ["Bool", "Char", "String", "Word", "Int", "Float", "Object"]
  elif formal == "Alpha":
    return actual in ["Char", "String", "Word"]
  elif formal == "Num":
    return actual in ["Int", "Float"]
  elif formal == "Alphanum":
    return actual in ["Char", "String", "Word", "Int", "Float"]
  elif formal == "Wordy":
    return actual in ["String", "Word"]
  elif formal == "Coll":
    return actual in ["Block", "List", "Seq"]
  elif formal == "Blocky":
    return actual in ["Block", "Seq"]
  elif formal == "Listy":
    return actual in ["List", "Block"]
  elif formal == "Otherwise":
    return false
  else:
...
proc handleError*(env: var Environment,  message: string, thing: LsNode)=
  ...
  if env.debug or env.verbose:
    ...
    echo " at line ", env.current.wordLine, ", column ", env.current.wordColumn

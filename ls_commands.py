...
####################### Listack commands
    ...
    def do_starship(self):      # number {do if positive} {do if zero} {do if negative} .<=>
        self.check_stack("<=>", 4)
...
# Boolean logic
    ...
    def do_less(self):  # a b .<
        self.check_stack("<", 2)
        self.check_types("<", ["alphanum", "same"])
        b = self.pop_past()
        a = self.pop_past()
        self.push_past(a < b)
    ...
    def do_equal(self):     # a b .=
        ...
        if is_bool(a) and is_bool(b):
            ...
            self.push_past(a == b)
        elif type(a) in [str, ls_word] and type(b) in [str, ls_word]:
            self.push_past(a == b)
        elif type(a) in [ls_list, ls_block] and type(b) in [ls_list, ls_block]:
            self.push_past(a == b)
        elif type(a) != type(b):
            self.push_past(False)
        else:
    ...
    def do_str_num(self):   # "123" --> 123
        ...
        if is_valid_number(a):
            if "." in a:
                self.push_past(float(a))
            else:
                self.push_past(int(a))
        else:

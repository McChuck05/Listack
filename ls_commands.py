...
# nop
    ...
    def do_reduce(self): # [list] {action} .reduce --> action progressively applied to list        *** action assumed to be postfix ***    uses side stacks e, r, s
        if len(self.past) < 2:
            print(f"\n{Fore.RED}Error: {Fore.CYAN}REDUCE{Fore.YELLOW}: stack is too low{Style.RESET_ALL}", flush = True)
            raise ValueError
        if not is_block(self.past[-1]):
            print(f"\n{Fore.RED}Error: {Fore.CYAN}REDUCE{Fore.YELLOW}: expected block to apply, not {Fore.RED}{self.past[-1]}{Style.RESET_ALL}", flush = True)
            raise ValueError
        if not is_list(self.past[-2]):
            print(f"\n{Fore.RED}Error: {Fore.CYAN}REDUCE{Fore.YELLOW}: expected list to work on, not {Fore.RED}{self.past[-2]}{Style.RESET_ALL}", flush = True)
            raise ValueError
        if len(self.past[-2]) == 0 or len(self.past[-1]) == 0:
            self.pop_past()
        else:
            self.meta_fut([0, 0, "push_r .extract_l push_s {pop_s swap copy_r .exec push_s} .each pop_r drop pop_s"])
...
# Boolean logic
    ...
    def do_str_num(self):   # "123" --> 123
        if len(self.past) < 1:
            print(f"\n{Fore.RED}Error: {Fore.CYAN}STR>NUM{Fore.YELLOW}: stack is too low{Style.RESET_ALL}", flush = True)
            raise ValueError
        a = self.pop_past()
        if not is_alpha(a):
    ...
    def do_num_str(self):   # 123 --> "123"
        if len(self.past) < 1:
            print(f"\n{Fore.RED}Error: {Fore.CYAN}NUM>STR{Fore.YELLOW}: stack is too low{Style.RESET_ALL}", flush = True)
            raise ValueError
        a = self.pop_past()
        if not is_number(a):
            print(f"\n{Fore.RED}Error: {Fore.CYAN}NUM>STR{Fore.YELLOW}: {Fore.MAGENTA}", end="")
            my_print(a, True)
            print(f"{Fore.YELLOW} is not a number{Style.RESET_ALL}", flush = True)
            raise ValueError
    ...
    def do_range(self): # start stop .range --> [start, next, next ... stop]
        if len(self.past) < 2:
            print(f"\n{Fore.RED}Error: {Fore.CYAN}RANGE{Fore.YELLOW}: stack is too low{Style.RESET_ALL}", flush = True)
            raise ValueError
        stop = self.pop_past()
        start = self.pop_past()
        result = ls_list([])

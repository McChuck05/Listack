...
######################  Meta processing
    ...
    def do_meta(self, raw_left, raw_right, raw_pattern):      # This is what you import
        ...
        pat = parse_commands(pattern, True)
    ...
    def do_load(self, verbose=False):
        ...
        if os.path.isfile(filename):
            if ext == "ls":
                ...
                parsedin = parse_commands(rawin, False)
        ...
        for i, what in enumerate(type_list):
            ...
            if correct == "SEQ":
                ...
                if prev_type in ["INT", "FLOAT"]:
                    correct_list = ["INT", "FLOAT"]
                    correct = "NUM"
                elif prev_type in ["LIST", "BLOCK"]:
                    correct_list = ["LIST", "BLOCK"]
                    correct = "SEQ"
...
####################### Listack commands
    ...
    def do_iff_pre(self):   # a iff: cond {true}
        self.ins_fut_2(ls_block([ls_word("nop")]))
        self.ins_fut_1(ls_word("if"))
        if is_block(self.future[0]):
            cond = self.pop_fut()
            self.ext_fut(cond)
    ...
    def do_iff_in(self):    # a cond iff {true}
        ...
        self.ins_fut_1(ls_block([ls_word("nop")]))
        self.push_fut(ls_word("if"))
    ...
    def do_iff_star_pre(self):   # a iff*: cond {true}  --> {a true} // nop
        self.check_stack("iff*:", 1)
        a = self.past[-1]
        # self.meta_fut([1, 2, "iff: %A {#a %B} {nop}"])
        self.meta_fut([1, 2, "%A if {#a %B} {nop}"])
        self.push_past(a)
    ...
    def do_iff_star_in(self):    # a cond iff* {true}  --> {a true} // nop
        ...
        cond = self.past[-1]
        if is_block(cond):
            self.meta_fut([2, 1, "%b if {#a %A} {nop}"])
            self.push_past(a)
        else:
            self.meta_fut([2, 1, "if {#a %A} {nop}"])
            self.push_past(a)
            self.push_past(cond)
    ...
    def do_starship_pre(self):
        self.ins_fut_4(ls_word(".<=>"))

    def do_starship_in(self):
        self.ins_fut_3(ls_word(".<=>"))
...
# else
    ...
    def do_while(self):    # postfix WHILE      condition body .while
        ...
        self.check_types("while", ["block", "block"])
    ...
    def do_until(self):     # {body} {condition} .do_until
        ...
        self.meta_fut([2, 0, "%a #b \.not .append #a .while"])
...
# nop
    ...
    def do_each(self):   # seq block .each        applies block to each item in seq     uses side stack e
        ...
        self.check_types("each", ["seq", "block"])
        self.meta_fut([1, 0, "push_e {pop_e .len 0 .>} {.first* swap push_e %a} .while drop"])
    ...
    def do_apply_each(self):   # [seq] [list of blocks] .apply_each        applies each block to each item in seq     uses side stacks d, e
        ...
        if not is_seq(self.past[-2]):
            self.past[-2] = ls_list([self.past[-2]])
        self.check_types("each", ["seq", "seq"])
        self.meta_fut([0, 0, "push_d dup push_e push_e {pop_d .len 0 .>}{.first* swap push_d pop_e swap .each} .while drop"])
    ...
    def do_filter(self):    # [list] {condition} .filter --> [filtered list]        uses side stacks f, g, e
        ...
        self.meta_fut([0, 0, "over swap .map swap push_g [] push_h {if {pop_g .first* swap push_g pop_h swap .concat push_h}{pop_g .first* drop push_g}} .each  pop_g drop pop_h"])
    ...
    def do_case(self):  # object [[{cond1}{body1}][{cond2}{body2}]...[True {default}]] .case        uses side stacks o, p
        ...
        self.meta_fut([0, 0, "[{True}{drop}] .append push_p push_o {pop_p .len 0 .>} while { .first* swap push_p copy_o swap .delist drop .type 'BLOCK' .~= { {} swap .append} .iff \\ break .append .iff} pop_o drop pop_p drop"])
    ...
    def do_case_star(self):  # object [[{cond1}{body1}][{cond2}{body2}]...[True {default}]] .case        uses side stacks o, p
        self.check_stack("case*", 2)
        self.check_types("case*", ["any", "seq"])
        self.meta_fut([0, 0, "[{True}{drop drop}] .append push_p dup push_o {pop_p .len 0 .>} while { .first* swap push_p copy_o swap .delist drop .type 'BLOCK' .~= { {} swap .append} .iff \\ break .append .iff} pop_o drop pop_p drop"])
    ...
    def do_match(self):  # object [[{cond1}{body1}][{cond2}{body2}]...[True {default}]] .match       uses side stacks o, p
        ...
        self.meta_fut([0, 0, "[{dup}{nop}] .append push_p push_o {pop_p .len 0 .>} while { .first* swap push_p copy_o swap .delist drop .type 'BLOCK' .~= { {} swap .append} .iff \\ break .append swap .type 'BLOCK' .~= { {} swap .append} .iff \\ .= .append swap .iff} pop_o drop pop_p drop"])
    ...
    def do_match_star(self):  # object [[{cond1}{body1}][{cond2}{body2}]...[True {default}]] .match*       uses side stacks o, p
        self.check_stack("match*", 2)
        # types can be anything, can be used as dictionary lookup
        self.meta_fut([0, 0, "[{dup}{nop}] .append push_p dup push_o {pop_p .len 0 .>} while { .first* swap push_p copy_o swap .delist drop .type 'BLOCK' .~= { {} swap .append} .iff \\ break .append swap .type 'BLOCK' .~= { {} swap .append} .iff \\ .= .append swap .iff} pop_o drop pop_p drop"])

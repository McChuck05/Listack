...
#######################
    ...
    def do_exec(self):     # {item}.exec // [item].exec // item.exec   -> item placed at front of command queue
        item = self.pop_past()
        if is_string(item) and item.count(" ") == 0:     # coerce string to command word.
            new = ls_word(item)
            if item.count(".") == 1:
                try:
                    new = float(item)
...
# nop
    ...
    def do_times(self):     # num {block} .times        uses side stack t
        if len(self.past) < 2:
        ...
        if not is_int(num):
            ...
            raise ValueError
        self.meta_fut([1, 0, "+ 1 push_t {pop_t - 1 dup push_t > 0} while #a pop_t drop"])
    ...
    def do_for(self):   # [{initial state}{incremental change}{exit condition}] {body} .for     uses side stack f
        ...
        if len(self.past[-2]) != 3:
            ...
            raise ValueError
        self.meta_fut([2, 0, "%a0 push_f {copy_f %a2 .~} while {%b pop_f %a1 push_f} pop_f drop"])  # [init inc cond] body --> init {cond} while {body inc}
                                            # counter is TOS for body to use
    ...
    def do_filter(self):    # [list] {condition} .filter --> [filtered list]
        self.meta_fut([0, 0, "over swap .map 2 (over drop [] $C {if {A .extract_l swap $A C swap .concat $C}{A .extract_l drop $A} } .each clear C )"])
    ...
    def do_total(self): # [list] {action} .total --> total of action applied to list        *** action assumed to be postfix ***
...
# Math
    ...
    def do_power(self):       # a b .^ --> a**b power or exponent
        if len(self.past) < 2:
            print(f"\n{Fore.RED}Error: {Fore.CYAN}POW{Fore.YELLOW}: stack is too low{Style.RESET_ALL}", flush = True)
            raise ValueError
        b = self.pop_past()
        a = self.pop_past()
        if not(is_number(a) and is_number(b)):
            print(f"\n{Fore.RED}Error: {Fore.YELLOW}POW expected numbers, not {Fore.CYAN}{a} {b}{Style.RESET_ALL}", flush = True)
            raise ValueError
        c = a**b
        self.push_past(c)

    def do_power_pre(self):  # ^: a b
        self.ins_fut_2(ls_word(".^"))

    def do_power_in(self):   # a ^ b
        self.ins_fut_1(ls_word(".^"))

    def do_root(self):  # a b .root --> a'th root of b
        if len(self.past) < 2:
    ...
    def do_rad>deg_pre(self):   # rad>deg: a
...
# Boolean logic
    ...
    def do_str_to_list_c(self):
        ...
        if not is_string(a):
            ...
            my_print(a)
            print(f" is not a string{Style.RESET_ALL}", flush = True)
            raise ValueError
        b = []
        for c in a:
            b.append(c)
